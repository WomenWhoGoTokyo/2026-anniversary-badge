...
| 14 | [ピンソケット 5Pin](https://akizukidenshi.com/catalog/g/g102762/)                                                                                        | 1 | ジョイスティック用 |
| 15 | ハット                                                                                                                                                | 1 | 0.61 g ジョイスティック用 |
| 16 | スイッチプレート                                                                                                                                           | 1 | 6.33 g |
| 17 | ボトムプレート                                                                                                                                            | 1 | 5.54 g |
| 18 | 木ねじ                                                                                                                                                | 2 | 2.1 × 10 |
...
RP2040 Zero 用の **9Pin × 2** と **5Pin × 1** を基板の表面に取り付けます。  
ピンソケットが浮いたり傾いたりすると、後で部品が斜めに刺さってしまいます。垂直に保つ工夫が必要です。

おすすめは、対応するピンヘッダー (RP2040 Zero やジョイスティック側の足) を一度ピンソケットに差し込んで、治具として使う方法です。これによりピンソケットがまっすぐ立ちます。
...
ST7789 液晶モジュールを基板の表面に取り付けます。  
液晶のフレキシブルケーブルや表示面にこてを当てないよう注意してください。

液晶モジュールに付属のピンヘッダを基板のスルーホールに差し込みます。  
**表示面が前 (バッジの正面) を向くように** 向きを確認してください。

{画像}

裏返して、まず 1 ピンだけはんだ付けします。  
液晶がまっすぐ取り付けられているかを表面側から確認し、傾いている場合ははんだを溶かして整えます。
...
ジョイスティックモジュールは Step 5 で取り付けた 5Pin ピンソケットに差し込むだけです。はんだ付けは不要です。
